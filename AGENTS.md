...
* **مدیریت الگوها در کلاستر بریف‌ها (`BriefsCluster`):**
  - **ساختار فرم ۳ تبِه (`BriefTemplateForm`):** 
    - *تب ۱ (مشخصات و تنظیمات)*: مدیریت نام الگو، وضعیت انتشار، فعال‌سازی حالت ویزاردی دسته‌بندی‌شده (`wizard_mode`) و متن پیام راهنمای بالای فرم (`guide_notice`).
    - *تب ۲ (طراح فیلدها و نیازمندی‌ها)*: ابزار Form Builder غنی با **۱۰ نوع بلوک بومی فیلامنت**:
      1. `text_input` (ورودی متن کوتاه)
      2. `textarea` (متن چندخطی)
      3. `select` (لیست کشویی)
      4. `radio_choice` (تک‌انتخابی رادیویی)
      5. `checkboxes` (چندانتخابی چک‌باکس با `CheckboxList` فیلامنت و آیکون `heroicon-o-queue-list`)
      6. `repeater` (فهرست پویا با دکمه `+` با استفاده از `Repeater` فیلامنت)
      7. `input_group` (گروه ورودی‌های متنی چندگانه برای فیلدهای مرتبط مانند نام فارسی/انگلیسی و اطلاعات تماس)
      8. `url_input` (ورودی آدرس سایت با اعتبارسنجی URL و پیشوند `https://`)
      9. `file_upload` (آپلود فایل با سوئیچ مدارک ضروری `is_essential`)
      10. `instruction_block` (باکس اطلاعیه و راهنما)
      - **فیلد دسته‌بندی گام‌ها (`step_title`):** امکان تعیین گام/دسته (مانند «گام اول: آشنایی اولیه»، «گام دوم: استراتژی و اهداف»...) روی تمام بلوک‌ها.
